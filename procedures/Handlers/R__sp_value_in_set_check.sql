...
BEGIN
    ...
    v_where_clause_condition := ''"'' || v_column_nm || ''" IS NOT NULL AND TRIM(CAST("'' || v_column_nm || ''" AS VARCHAR)) != '''''''' AND TRIM(CAST("'' || v_column_nm || ''" AS VARCHAR)) NOT IN ('' || v_in_set_condition || '')'';
    v_match_condition := ''"'' || v_column_nm || ''" IS NOT NULL AND TRIM(CAST("'' || v_column_nm || ''" AS VARCHAR)) != '''''''' AND TRIM(CAST("'' || v_column_nm || ''" AS VARCHAR)) IN ('' || v_in_set_condition || '')'';
