...
BEGIN
    ...
    v_where_clause_condition := ''"'' || v_column_nm || ''" IS NOT NULL AND TRIM("'' || v_column_nm || ''"::STRING) <> '''''''' AND (LEN("'' || v_column_nm || ''") < '' || COALESCE(v_min_length::STRING, ''0'') || 
                                 '' OR LEN("'' || v_column_nm || ''") > '' || COALESCE(v_max_length::STRING, ''9999999'') || '')'';

    v_sql := ''SELECT COUNT(*) AS total_count, COUNT_IF("'' || v_column_nm || ''" IS NULL OR TRIM("'' || v_column_nm || ''"::STRING) = '''''''') AS missing_count, COUNT_IF('' || v_where_clause_condition || '') AS unexpected_count FROM '' || v_from_clause || '' WHERE 1=1 '' || v_incremental_filter;
    ...
    IF (v_error_message IS NULL) THEN
        BEGIN
            ...
            v_unexpected_percent_nonmissing := v_percent;
    ...
    IF (v_error_message IS NULL) THEN
        BEGIN
            ...
            v_sql := ''INSERT INTO "'' || v_dq_db_name || ''"."'' || v_dq_schema_name || ''".DQ_RULE_RESULTS (BATCH_ID, DATASET_RUN_ID, DATASET_ID, RULE_CONFIG_ID, EXPECTATION_ID, RUN_NAME, RUN_TIMESTAMP, DATASET_NAME, EXPECTATION_CONFIG, IS_SUCCESS, RESULTS, EXPECTATION_NAME, DETAILS, ELEMENT_COUNT, MISSING_COUNT, MISSING_PERCENT, OBSERVED_VALUE, PARTIAL_UNEXPECTED_COUNTS, PARTIAL_UNEXPECTED_INDEX_LIST, PARTIAL_UNEXPECTED_LIST, UNEXPECTED_COUNT, UNEXPECTED_PERCENT, UNEXPECTED_PERCENT_NONMISSING, UNEXPECTED_PERCENT_TOTAL, UNEXPECTED_ROWS, DATA_ROWS, FAILED_ROWS, DIMENSION) SELECT '' || COALESCE(v_batch_id::STRING, ''null'') || '', '' || COALESCE(v_run_id::STRING, ''null'') || '', '' || COALESCE(v_data_asset_id::STRING, ''null'') || '', '' || COALESCE(v_check_config_id::STRING, ''null'') || '', '' || COALESCE(v_expectation_id::STRING, ''null'') || '', '''''' || REPLACE(COALESCE(v_run_name, ''null''), '''''''', '''''''''''') || '''''', CURRENT_TIMESTAMP(), '''''' || REPLACE(COALESCE(v_data_asset_name, ''null''), '''''''', '''''''''''') || '''''', PARSE_JSON('''''' || REPLACE(COALESCE(RULE::STRING, ''null''), '''''''', '''''''''''') || ''''''), '' || CASE WHEN v_status_code = v_success_code THEN ''TRUE'' ELSE ''FALSE'' END || '', PARSE_JSON('''''' || REPLACE(results_json_str, '''''''', '''''''''''') || ''''''), '''''' || REPLACE(COALESCE(v_expectation_name, ''null''), '''''''', '''''''''''') || '''''', PARSE_JSON(''''{}''''), '' || COALESCE(v_total::STRING, ''null'') || '', '' || COALESCE(v_missing_count::STRING, ''null'') || '', '' || COALESCE(v_missing_percent*100::STRING, ''null'') || '', NULL::VARIANT, PARSE_JSON(\\'''' || REPLACE(COALESCE(v_value_counts::STRING, ''null''), ''\\'''', '''''''''''') || ''\\''), NULL::VARIANT, NULL::VARIANT, '' || COALESCE(v_unexpected::STRING, ''null'') || '', '' || COALESCE(v_percent*100::STRING, ''null'') || '', '' || COALESCE(v_unexpected_percent_nonmissing*100::STRING, ''null'') || '', '' || COALESCE(v_unexpected_percent_total*100::STRING, ''null'') || '', PARSE_JSON(\\'''' || REPLACE(COALESCE(v_column_profile::STRING, ''null''), ''\\'''', '''''''''''') || ''\\''), NULL::VARIANT, NULL::VARIANT, NULL::VARIANT'';
            EXECUTE IMMEDIATE v_sql;
        EXCEPTION
            WHEN OTHER THEN
